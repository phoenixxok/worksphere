  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
      <div className="flex justify-between items-baseline">
        <p className="text-sm font-semibold text-slate-900 uppercase tracking-wide">Transparent split</p>
      ...
      {rows.map(([label, value, pct, color]) => (
        <div key={label} className="flex justify-between text-sm">
          ...
          <span className="font-semibold tabular-nums text-slate-900">{rupees(value)}</span>
        </div>
      ))}

      <div className="flex justify-between text-sm border-t border-slate-200 pt-2">
        <span className="text-slate-500">Total</span>
        <span className="font-semibold tabular-nums text-slate-900">{rupees(payment.total_amount_inr)}</span>
      </div>
    </div>
  );
}
